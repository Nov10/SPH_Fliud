...
public class ParticleDisplay3D : MonoBehaviour
{
    ...
    public void Init(ComputeBuffer position, ComputeBuffer velocity)
    {
        mat = new Material(shader);
        mat.SetBuffer("Positions", position);
        mat.SetBuffer("Velocities", velocity);

        mesh = SebStuff.SphereGenerator.GenerateSphereMesh(meshResolution);
        debug_MeshTriCount = mesh.triangles.Length / 3;
        argsBuffer = ComputeHelper.CreateArgsBuffer(mesh, position.count);
