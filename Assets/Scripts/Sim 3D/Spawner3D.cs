...
public class Spawner3D : MonoBehaviour
{
    ...
    public Vector3 BoundaryConditionCenterPosition;
    public float size;
        ...
        for (int x = 0; x < numParticlesPerAxis; x++)
        {
            for (int y = 0; y < numParticlesPerAxis; y++)
            {
                for (int z = 0; z < numParticlesPerAxis; z++)
                {
                    ...
                    float px = (tx - 0.5f) * size + BoundaryConditionCenterPosition.x;
                    float py = (ty - 0.5f) * size + BoundaryConditionCenterPosition.y;
                    float pz = (tz - 0.5f) * size + BoundaryConditionCenterPosition.z;
    ...
    void OnDrawGizmos()
    {
        if (showSpawnBounds && !Application.isPlaying)
        {
            ...
            Gizmos.DrawWireCube(BoundaryConditionCenterPosition, Vector3.one * size);
        }
    }
}
